...
def create_index_from_folder(folder_path: str) -> str:
    index_persistent_path = Path(INDEX_DIR) / Path(folder_path).name
    ...
    with acquire_lock(lock_path):
        ...
        for pdf_file in Path(folder_path).rglob("*.pdf"):
            ...
            log(f"Processing file: {pdf_path}")
            index.insert_batch_from_pdf(pdf_path)
...
if __name__ == "__main__":
    docs_folder = os.path.join(parent_path, "data", "docs")
    create_index_from_folder(docs_folder)